...
This RFC defines how activeadmin_batched_export design changes are proposed, written, numbered, and advanced.

## Motivation

The workspace URL, `export_meta` JSON, batch chunk formats, and Stimulus assembly are the public contract. Hosts pin Stimulus controllers and copy partials against those names. Numbered RFCs are the unit for a change that would move the gem version during an operator-owned rewrite.
